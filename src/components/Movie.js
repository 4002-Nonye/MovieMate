...
const Movie = ({ movie }) => {
  if (!movie) {
    return <div className="text-white text-xl flex justify-center items-center m-auto text-center h-56">oops! Nothing Found😐</div>;
  }
        ...
        return (
          <div key={movieReq.id}>
            <div className="mb-3">
              ...
              <div className="mt-3">
                {" "}
                <p className="text-slate-300">Movie Title- {movieReq.title}</p>
              ...
              <button className="bg-slate-900 p-2 rounded-md mt-2 text-white hover:text-red-400">View more</button>
            </div>
            </div>
          </div>
        );
